...
#from utils_model import generate_plant_sequence
...
def encode_per_timestep(encoder, X_seq,seq_len,fea_dim):
    """
...
def rollout_meta_sarsa(meta_model,seq_len,fea_dim, steps=30, epsilon=0.1):
    """
    X_seq: [T, F] 连续特征序列 (temp, humid, light, ac, heater, dehum, hum)
    labels: 可选，用于计算 reward
    """
    # FIX: 不使用整体 embedding；按时间步编码
    s_latent_seq = encode_per_timestep(meta_model, X_train,seq_len,fea_dim)  # [T, FEATURE_DIM]
    ...
    for t in range(steps):
        ...
        if t + 1 < X_train.shape[0]:
            ...
            # 同步下一步 latent
            s_latent_seq[t + 1] = encode_per_timestep(meta_model, X_train[t + 1:t + 2],seq_len,fea_dim)[0]
...
def serv_pipline(num_classes=NUM_CLASSES, seq_len=SEQ_LEN, num_feats=NUM_FEATURES, feature_dim=FEATURE_DIM):
    ...
    sarsa_full_batch_robust(encoder,seq_len,num_feats)
    #test_rollout(encoder)
    #rollout_meta_sarsa(encoder, steps=30, epsilon=0.1)
...
# ------------------ 完全批量化 SARSA 训练 ------------------
def sarsa_full_batch_robust(encoder,seq_len,num_feats):
    global eps
    ...
    for ep in range(1, SARSA_EPISODES + 1):
        ...
        for start_idx in range(0, T - 1, BATCH_MAX):
            ...
            s_latent_seq = encode_per_timestep(encoder, raw_seq,seq_len,num_feats)
            ...
            try:
                q_pred = q_net.predict(s_latent_seq[:-1], verbose=0)
            except:
            ...
            #
            #raw_next_seq[:, 3:3 + NUM_SWITCH] = actions_bits
            s_next_latent_seq = encode_per_timestep(encoder, raw_next_seq,seq_len,num_feats)
    ...
    save_sarsa_tflite(q_net)

# ------------------ 测试 rollout ------------------
def test_rollout(encoder, steps=30):
    df = pd.read_csv(np.random.choice(files))

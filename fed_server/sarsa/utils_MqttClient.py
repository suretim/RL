...
class MqttClientServer(mqtt_client.Client):
    ...
    def load_ewc_assets(self,save_dir):
    ...
    def save_ewc_assets_to_bin(self ,save_dir):
        # Load model weights
    ...
    def publish_message(self,save_dir):
        fisher_matrix = self.load_ewc_assets(save_dir)
    ...
    def _on_message(cls,client,  userdata, msg):
        ...
        try:
            ...
            if response.update_successful is True:
                payload_weights, payload_bias = client.fserv.federated_avg(data_dir=client.data_dir, device_id=client_id)

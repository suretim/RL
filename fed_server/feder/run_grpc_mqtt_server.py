...
def main(args):
    ...
    try:
        ...
        # 创建 MQTT 客户端
        mq_client = MqttClientServer(fserv=fserv ,
                                       mqtt_broker=args.mqtt_broker,
                                       mqtt_port=args.mqtt_port,
                                       data_dir=args.ewc_assets)
        #mq_client.on_connect = MqttClientServer._on_connect
        #mq_client.on_message = MqttClientServer._on_message
        # 设置用户名和密码
        #username = "tim"  # 替换为你的 MQTT 用户名
        #password = "tim"  # 替换为你的 MQTT 密码
        #mq_client.username_pw_set(username, password)  # 设置用户名和密码
        # 设置重连超时时间，单位为毫秒
        #reconnect_timeout_ms = 10000  # 10秒的重连超时
        #mq_client.reconnect_delay_set(min_delay=1, max_delay=10)  # 设置重连延迟（最小1秒，最大10秒）

        #mqtt_client.connect(MQTT_BROKER, MQTT_PORT, 60)
        mq_client.start_connect()

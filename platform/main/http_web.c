...
unsigned char cmd_interpreter_en = 1;
...
static void process_command(httpd_req_t *req, const char *command) {
    ...
    if(strcmp(command, "help") == 0) {
        sprintf(response, "help fps=? detect=1/0");
    }
    else if(strcmp(command, "fps=?") == 0) {
        sprintf(response, "fps=%.1f", frame_rate);
    }    
    else if(strcmp(command, "detect=1") == 0) {
        cmd_interpreter_en = 1;
        return;
    }  
    else if(strcmp(command, "detect=0") == 0) {
        cmd_interpreter_en = 0;
        return;
    }   
...
void video_stream_task(void *pvParameters) {
    ...
    static unsigned int infer_tmr = 0;
    ...
    infer_tmr = tick_get();                   // <--- initialize timer
    while (1) {        
        ...
        // <--- NEW: check every loop whether 60 seconds have passed
        if (tick_cmp(infer_tmr, 60000) == c_ret_ok) {
            if(cmd_interpreter_en==1)
                interpreter_en = 1;               // trigger inference flag
            infer_tmr = tick_get();           // reset 60s timer
            ESP_LOGI(TAG, "Enabled interpreter_en (every 60s)");
        }
        if(fb->format == PIXFORMAT_JPEG)
        {
            if (xSemaphoreTake(web_send_mutex, portMAX_DELAY) == pdTRUE) {
